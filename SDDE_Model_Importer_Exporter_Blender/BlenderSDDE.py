...
def write_bone_palette(mesh_name, collection_name, bone_names):
    BP = io.BytesIO()
    ...
    BonePaletteID = Custom_Hash_Function(mesh_name + ".BonePalette")
...
def write_index_buffer(mesh_name, mesh_obj):
    IB = io.BytesIO()
    ...
    IndexBufferID = Custom_Hash_Function(mesh_name + ".IndexBuffer")
    ...
    base = (mesh_name + ".IndexBuffer").encode('ascii')
    ...
    IB.write(struct.pack("<I", IndexCount * 2)) # NumBytes
    
    IB.write(b'\xD0\x00\x00\x00\x00\x00\x00\x00')
    
    IB.write(struct.pack("<I", 2)) # IndexStride
    
    IB.write(struct.pack("<I", IndexCount)) # IndexCount
...
def write_vertex_buffer(mesh_name, buffer_index, VertexStride, bytes_preserved, mesh_obj, BoneIndexMap):
    VB = io.BytesIO()
     
    VertexCount = len( mesh_obj.data.vertices)
    
    paddingVB = struct.pack("<I", align_to_16(VB))
    
    VB.write(b'\x00' * 24)

    buffer_id = Custom_Hash_Function(f"{mesh_name}.VertexBuffer{buffer_index}.0")
    VB.write(struct.pack("<I", buffer_id))
    
    VB.write(b'\x00' * 20)
    VB.write(b'\x8F\xEC\xCD\x92')

    name_base = f"{mesh_name}.VertexBuffer{buffer_index}.0".encode('ascii')
    VB.write(name_base + b'\x00' * (36 - len(name_base)))
    
    VB.write(b'\x01\x00\x00\x00')
    
    VB.write(struct.pack("<I", VertexCount * VertexStride))
    
    VB.write(b'\xD0\x00\x00\x00\x00\x00\x00\x00')
    
    VB.write(struct.pack("<I", VertexStride))
    
    VB.write(struct.pack("<I", VertexCount))
...
def write_model_table(collection_name, BonePaletteID, mesh_count, MaterialNameHash, VertexDeclarationID, IndexBufferID, VertexBuffer_0_ID, VertexBuffer_1_ID, VertexBuffer_2_ID, PrimitiveCount, AABBMin, AABBMax, IndexCounts):
    ...
    paddingMT = struct.pack("<I", align_to_16(MT))
        ...
        for model_index, model_table in enumerate(Model_Tables):
            ...
            for mesh_index in range(model_table.MeshCount):   
                ...
                for vb in [Vertex_Buffer0, Vertex_Buffer1, Vertex_Buffer2]:
                    ...
                    if hasattr(vb, "UVs0") and vb.UVs0:
                        UV_Buffers.append(vb.UVs0)

                    if hasattr(vb, "UVs1") and vb.UVs1:
                    ...
                    if hasattr(vb, "Colors") and vb.Colors and Colors_Buffer is None:
                        Colors_Buffer = vb
                ...
                positions = Positions_Buffer.Positions
                ...
                if Index_Buffer:
                    ...
                    faces = [(Mesh_Indices[i], Mesh_Indices[i+1], Mesh_Indices[i+2])
...
#                    mesh_data.update()
                ...
                if UV_Buffers:
                    for i, uv_data in enumerate(UV_Buffers):
                        uv_layer = MeshData.uv_layers.new(name=f"UVMap_{i}")
                        for poly in MeshData.polygons:
                            for loop_index in range(poly.loop_start, poly.loop_start + poly.loop_total):
                                vertex_index = MeshData.loops[loop_index].vertex_index
                                u, v = uv_data[vertex_index]
                                uv_layer.data[loop_index].uv = (float(u), float(v))

            
                
                if Colors_Buffer:
                    color_layer = MeshData.vertex_colors.new(name="Col")
                    for poly in MeshData.polygons:
                        for loop_index in range(poly.loop_start, poly.loop_start + poly.loop_total):
                            vertex_index = MeshData.loops[loop_index].vertex_index
                            r, g, b, a = Colors_Buffer.Colors[vertex_index]
                            color_layer.data[loop_index].color = (r, g, b, a)

                
                if Weights_Buffer and BonePalette:
                    for i, bone_name in enumerate(BonePalette.BoneNames):
                        MeshObj.vertex_groups.new(name=bone_name)
                        

                    for vertex_index, bone_weight_list in enumerate(Weights_Buffer.BlendIndicesAndWeights):
                        for blend_index, blend_weight in bone_weight_list:
                                MeshObj.vertex_groups[blend_index].add([vertex_index], blend_weight, 'ADD')
...
#    output_path = os.path.join(filepath, filename_no_ext + ".perm.bin")
    ...
    with open(filepath + ".perm.bin", "wb") as out:
        ...
        for collection in collections_to_export:  
            ...
            for i, mesh_obj in enumerate(mesh_objs):
                   
                paddingMat, MaterialChunk, MaterialNameHash= write_material_chunk(mesh_names[i])

                paddingBP, BonePalette, BonePaletteID, BoneIndexMap = write_bone_palette(mesh_names[i], collection_name, bone_names)
                
                paddingIB, bytes_preserved, IndexBuffer, IndexBufferID, PrimitiveCount, IndexCounts = write_index_buffer(mesh_names[i], mesh_objs[i])
                
                paddingVB0, VertexBuffer0, VertexBuffer_0_ID, AABBMin, AABBMax = write_vertex_buffer(mesh_names[i], 0, 24, bytes_preserved, mesh_objs[i], BoneIndexMap)
                
                paddingVB1, VertexBuffer1, VertexBuffer_1_ID, AABBMin, AABBMax = write_vertex_buffer(mesh_names[i], 1, 8, bytes_preserved, mesh_objs[i], BoneIndexMap)
                
                paddingVB2, VertexBuffer2, VertexBuffer_2_ID, AABBMin, AABBMax = write_vertex_buffer(mesh_names[i], 2, 4, bytes_preserved, mesh_objs[i], BoneIndexMap)
